/**
 * GitHub Issues Creation Script for AnKa Web App
 * Repository: akashroy8210/Anka-web-App
 * 
 * Usage:
...
const GITHUB_REPO = 'akashroy8210/Anka-web-App';
const GITHUB_TOKEN = process.argv[2] || process.env.GITHUB_TOKEN;

if (!GITHUB_TOKEN) {
  console.error('❌ Error: GitHub Token required.');
  console.log('Usage: node scripts/create_github_issues.js <GITHUB_TOKEN>');
  process.exit(1);
}
...
### Affected Files
- \`server/controllers/payments.js\`
- \`server/services/whatsappService.js\` [NEW]`
  },
  {
...
- \`server/controllers/auth.js\``
  ...
  {
    title: '[MEDIUM] Strict File Upload Validation & Media Quotas (Uploads Route)',
...
### Affected Files
- \`server/models/SurpriseInstance.js\`
- \`server/scripts/archiveExpiredInstances.js\` [NEW]`
  },
  {
...
async function createIssue(issue) {
  return new Promise((resolve, reject) => {
    ...
    const options = {
      ...
      method: 'POST',
      headers: {
        'User-Agent': 'Anka-Issue-Creator-Script',
    ...
    const req = https.request(options, (res) => {
      ...
      res.on('end', () => {
        if (res.statusCode === 201) {
          ...
          console.log(`✅ Issue Created #${resObj.number}: ${resObj.title}`);
          resolve(resObj);
        } else {
          console.error(`❌ Failed to create issue '${issue.title}': Status ${res.statusCode} - ${body}`);
          resolve(null);
        }
      });
    });
...
async function run() {
  console.log(`🚀 Creating ${issues.length} GitHub Issues for repository ${GITHUB_REPO}...`);
  for (const issue of issues) {
    await createIssue(issue);
    // Small delay to prevent GitHub rate limits
    await new Promise(r => setTimeout(r, 1000));
  }
  console.log('🎉 Done creating GitHub issues!');
}
