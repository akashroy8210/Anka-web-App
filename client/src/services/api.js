...
export const api = {
  ...
  generateFeedingImage: async (instanceId, malePhotoUrl, femalePhotoUrl, token) => {
    const res = await fetch(`${API_BASE}/instances/${instanceId}/generate-feeding-image`, {
      method: 'POST',
      headers: getHeaders(token),
      body: JSON.stringify({ malePhotoUrl, femalePhotoUrl })
    });
    return res.json();
  },
